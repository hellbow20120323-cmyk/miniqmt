...
**实现状态**：Windows 端已实现 [global_vault.py](global_vault.py)（单例 + 文件锁），[order_executor.py](order_executor.py) 已扩展双标的与 Vault 审批（ALLOWED_CODES 含 512890.SH，买入前 request_allocation、成交后 on_fill、卖出成功后 release、失败/撤单 on_cancel）。Mac 端 StrategyNodes 与 shared_pool 读取逻辑待实现。
...
|------|------|------|------|
| signal_id | 是 | — | 幂等与消费用 |
| client_order_id | 否 | signal_id | Vault 与 release 用，建议与 signal_id 一致或一一对应 |
