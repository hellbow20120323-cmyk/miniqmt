"""
512890 红利低波 ETF — ATR 动态网格 + 趋势自适应回测
与 159201 同套逻辑，针对低波动品种调参：步长略密、止盈略低。数据来自 QMT（quote_512890_1min_5y.csv，5 年）。
"""
import pandas as pd
import numpy as np
...
# --- [1] 参数配置（512890 低波适配，见 策略文档_512890.md）---
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
# 优先 5 年 → 3 年 → 2024 年（需先运行 fetch_quote_512890.py 拉取）
_FILE_5Y = os.path.join(_SCRIPT_DIR, 'quote_512890_1min_5y.csv')
_FILE_3Y = os.path.join(_SCRIPT_DIR, 'quote_512890_1min_3y.csv')
_FILE_2024 = os.path.join(_SCRIPT_DIR, 'quote_512890_1min_2024.csv')
FILE_PATH = _FILE_5Y if os.path.exists(_FILE_5Y) else (_FILE_3Y if os.path.exists(_FILE_3Y) else _FILE_2024)
OUTPUT_IMAGE = os.path.join(_SCRIPT_DIR, 'optimized_backtest_512890.png')
...
# 512890 低波适配（默认 baseline：网格与冷静期偏保守；tune 时 combo2 为更密网格+关冷静期）
ATR_GRID_FACTOR = 0.32           # cooling_off：与 tune 中 210 笔一致；0.30 时再开仓慢、仅 ~68 笔
GRID_STEP_FLOOR = 0.001         # baseline：步长下限 0.10%（combo2 为 0.08%）
LAYER_STEP_BONUS = 0.0001       # 层数加成
SELL_PROFIT_THRESHOLD = 0.004   # 止盈阈值 0.4%
SELL_BY_LOT = True
BUY_STEP_FACTOR = 1.0
SELL_THRESHOLD_FACTOR = 1.35
...
# 动态冷静期（默认 cooling_off：关闭冷静期 + 保留保守网格，收益/回撤更均衡；baseline 为 True）
COOLING_ENABLED = False             # cooling_off：关冷静期，年化约 216 笔、回撤约 -0.77%、Alpha 约 1.2 万
COOLING_BARS = 15                    # 若 COOLING_ENABLED=True 时生效
COOLING_BARS_SHORT = 5               # RSI<35 时缩短
...
def run_backtest(
    ...
    grid_step_floor_override=None,
    batch_money_override=None,
    beta_capital_override=None,
    atr_grid_factor_override=None,
    cooling_enabled_override=None,
    cooling_bars_override=None,
    cooling_bars_short_override=None,
    uptrend_grid_factor_override=None,
    uptrend_sell_factor_override=None,
    uptrend_batch_factor_override=None,
    downtrend_grid_factor_override=None,
    downtrend_sell_factor_override=None,
    downtrend_batch_factor_override=None,
    layer_weights_override=None,
    sell_profit_threshold_override=None,
    layer_step_bonus_override=None,
):
    """return_metrics=True 时只返回 dict。layer_weights_override 为 9 元组时替代 LAYER_WEIGHTS。"""
    if not os.path.exists(FILE_PATH):
        print(f"错误: 未找到数据文件 {FILE_PATH}")
        print("请先运行 fetch_quote_512890.py（Windows/QMT）拉取 512890 分钟数据，或将该 CSV 放到脚本同目录。")
        return

    # --- [2] 加载与处理数据（支持 QMT 导出的 time 列为毫秒时间戳）---
    ...
    # 诊断：实际生效的网格/冷静期参数（便于核对为何单次回测与 tune 中 cooling_off 笔数不一致）
    _floor = grid_step_floor_override if grid_step_floor_override is not None else GRID_STEP_FLOOR
    _atr = atr_grid_factor_override if atr_grid_factor_override is not None else ATR_GRID_FACTOR
    _cool = cooling_enabled_override if cooling_enabled_override is not None else COOLING_ENABLED
    _start, _end = df.index.min(), df.index.max()
    _days = (_end - _start).days if hasattr(_end - _start, 'days') else 0
    if not return_metrics:
        print(f"[数据] 文件: {os.path.basename(FILE_PATH)} | 区间: {_start.date()} ~ {_end.date()} | 约 {_days/365:.1f} 年 | {len(df)} 根K线")
        print(f"[参数] ATR_GRID_FACTOR={_atr} GRID_STEP_FLOOR={_floor} COOLING_ENABLED={_cool}")
    ...
    if max_layers_override is not None:
        ...
        _weights = layer_weights_override if (layer_weights_override is not None and len(layer_weights_override) == MAX_LAYERS) else LAYER_WEIGHTS
        _weight_sum = sum(_weights)
        part_money_list = [batch_money * w / _weight_sum for w in _weights]
    ...
    for i in range(len(df)):
        ...
        # 动态步长与止盈（支持 tune 覆盖）
        _floor = grid_step_floor_override if grid_step_floor_override is not None else GRID_STEP_FLOOR
        _atr_factor = atr_grid_factor_override if atr_grid_factor_override is not None else ATR_GRID_FACTOR
        base_grid_step = max(_floor, (atr / curr_p) * _atr_factor)
        sell_threshold = sell_profit_threshold_override if sell_profit_threshold_override is not None else SELL_PROFIT_THRESHOLD
        batch_factor = 1.0

        ug = uptrend_grid_factor_override if uptrend_grid_factor_override is not None else UPTREND_GRID_FACTOR
        us = uptrend_sell_factor_override if uptrend_sell_factor_override is not None else UPTREND_SELL_FACTOR
        ub = uptrend_batch_factor_override if uptrend_batch_factor_override is not None else UPTREND_BATCH_FACTOR
        dg = downtrend_grid_factor_override if downtrend_grid_factor_override is not None else DOWNTREND_GRID_FACTOR
        ds = downtrend_sell_factor_override if downtrend_sell_factor_override is not None else DOWNTREND_SELL_FACTOR
        db = downtrend_batch_factor_override if downtrend_batch_factor_override is not None else DOWNTREND_BATCH_FACTOR
        ...
        _layer_bonus = layer_step_bonus_override if layer_step_bonus_override is not None else LAYER_STEP_BONUS
        grid_step = base_grid_step + (hold_layers * _layer_bonus)
        ...
        # 买入逻辑 (滚动锚点)，熔断/时间窗口/冷静期时不加仓（支持 tune 覆盖冷静期）
        _cooling_enabled = cooling_enabled_override if cooling_enabled_override is not None else COOLING_ENABLED
        _cooling_bars = cooling_bars_override if cooling_bars_override is not None else COOLING_BARS
        _cooling_bars_short = cooling_bars_short_override if cooling_bars_short_override is not None else COOLING_BARS_SHORT
        if hold_layers < max_layers and buy_triggered and not pause_buy and not time_filter_block_buy:
            in_cooling = False
            if _cooling_enabled and hold_layers == 0 and hold_t0_volume == 0:
                rsi_now = df['rsi'].iloc[i] if pd.notna(df['rsi'].iloc[i]) else 50
                cooling_bars_effective = _cooling_bars_short if rsi_now < RSI_COOLING_THRESHOLD else _cooling_bars
    ...
    plt.title('512890 Dividend Low-Vol Grid - Alpha')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "tune":
        # 调参实验：网格压缩 / 冷静期缩短或关闭 / 趋势系数优化，对比成交笔数与 Alpha
        # 先打印数据区间，避免把“区间总笔数”误当“年化笔数”
        if os.path.exists(FILE_PATH):
            _cols = pd.read_csv(FILE_PATH, nrows=0).columns.tolist()
            if 'time' in _cols:
                _t = pd.read_csv(FILE_PATH, usecols=['time'])
                _t['time'] = pd.to_datetime(_t['time'], unit='ms', errors='coerce')
                _start, _end = _t['time'].min(), _t['time'].max()
            else:
                _full = pd.read_csv(FILE_PATH, index_col=0, parse_dates=True)
                _start, _end = _full.index.min(), _full.index.max()
            _days = (_end - _start).days if hasattr(_end - _start, 'days') else 0
            _y = _days / 365.0 if _days > 0 else 0
            print("\n[tune] 数据区间: %s ~ %s | 约 %.2f 年 | 成交笔数 = 该区间内总笔数（非年化）" % (_start.date(), _end.date(), _y))
        configs = [
            ("baseline", {}),
            ("grid_tight", {"grid_step_floor_override": 0.0008, "atr_grid_factor_override": 0.28}),
            ("grid_tighter", {"grid_step_floor_override": 0.0006, "atr_grid_factor_override": 0.26}),
            ("cooling_short", {"cooling_bars_override": 5, "cooling_bars_short_override": 2}),
            ("cooling_off", {"cooling_enabled_override": False}),
            ("trend_opt", {"downtrend_grid_factor_override": 0.88, "uptrend_sell_factor_override": 1.15}),
            ("combo1", {"grid_step_floor_override": 0.0008, "atr_grid_factor_override": 0.28, "cooling_bars_override": 5, "cooling_bars_short_override": 2}),
            ("combo2", {"grid_step_floor_override": 0.0008, "atr_grid_factor_override": 0.28, "cooling_enabled_override": False}),
            ("combo3", {"grid_step_floor_override": 0.0008, "atr_grid_factor_override": 0.28, "cooling_bars_override": 5, "cooling_bars_short_override": 2, "downtrend_grid_factor_override": 0.88, "uptrend_sell_factor_override": 1.15}),
            # 超高频 (Extreme High-Freq)：止盈 0.2%、步长 0.05%、无层数加成、ATR 0.15，观察年化是否突破 300 笔
            ("extreme_high_freq", {
                "sell_profit_threshold_override": 0.002,
                "grid_step_floor_override": 0.0005,
                "layer_step_bonus_override": 0.0,
                "atr_grid_factor_override": 0.15,
                "cooling_enabled_override": False,
            }),
        ]
        results = []
        for name, kw in configs:
            m = run_backtest(return_metrics=True, **kw)
            if m is None:
                continue
            m["name"] = name
            # 综合分：成交笔数归一化(每10笔1分) + Alpha归一化(每1000元0.5分) - 回撤%惩罚
            alpha_norm = m["alpha"] / 1000.0 * 0.5
            dd_penalty = abs(min(0, m["max_dd"] * 100))
            m["score"] = (m["n_trades"] / 10.0) * 0.3 + alpha_norm * 0.5 - dd_penalty * 0.02
            results.append(m)
        if not results:
            print("tune: 无有效回测结果（请检查数据文件）")
        else:
            print("\n512890 tune 调参实验（提升交易次数与 Alpha）")
            print("-" * 120)
            print(f"{'配置':<18} | {'成交笔数':>8} | {'年化笔数':>8} | {'Alpha(元)':>10} | {'最大回撤%':>10} | {'占用年化%':>10} | {'打满':>4} | 综合得分")
            print("-" * 120)
            for r in results:
                _y = r.get("years_span") or 1.0
                _tpy = r["n_trades"] / _y if _y > 0 else 0
                print(f"{r['name']:<18} | {r['n_trades']:>8} | {_tpy:>8.0f} | {r['alpha']:>10,.0f} | {r['max_dd']*100:>10.2f} | {r['flow_ann_util']*100:>10.2f} | {r['exhaust_count']:>4} | {r['score']:.2f}")
            print("-" * 120)
            best = max(results, key=lambda x: x["score"])
            _best_tpy = best["n_trades"] / (best.get("years_span") or 1.0)
            print(f"综合得分最优: {best['name']} (成交 {best['n_trades']} 笔, 年化约 {_best_tpy:.0f} 笔, Alpha {best['alpha']:,.0f} 元, 回撤 {best['max_dd']*100:.2f}%)")
            print("说明: 年化笔数 = 成交笔数/数据年数；extreme_high_freq 用于观察是否突破 300 笔/年。")
    elif len(sys.argv) > 1 and sys.argv[1] == "cooling_off":
        # 仅跑 cooling_off（与 tune 表里 cooling_off 完全同参），便于对比笔数
        run_backtest(
            return_metrics=False,
            cooling_enabled_override=False,
            atr_grid_factor_override=0.32,
            grid_step_floor_override=0.001,
        )
        print("\n（本 run 与 tune 中 cooling_off 同参：ATR=0.32, floor=0.001, cooling=False）")
    elif len(sys.argv) > 1 and sys.argv[1] == "compare_atr":
        # ATR_GRID_FACTOR 0.30 vs 0.32 对比（其余同 cooling_off）
        results = []
        for atr_val, label in [(0.30, "ATR=0.30"), (0.32, "ATR=0.32")]:
            m = run_backtest(
                return_metrics=True,
                cooling_enabled_override=False,
                atr_grid_factor_override=atr_val,
                grid_step_floor_override=0.001,
            )
            if m is not None:
                m["name"] = label
                results.append(m)
        if len(results) >= 2:
            print("\n512890 ATR_GRID_FACTOR 对比（cooling_off 同参，仅改 ATR）")
            print("-" * 95)
            print(f"{'ATR':<12} | {'成交笔数':>8} | {'年化笔数':>8} | {'Alpha(元)':>10} | {'最大回撤%':>10} | {'占用年化%':>10} | {'打满':>4}")
            print("-" * 95)
            for r in results:
                _y = r.get("years_span") or 1.0
                _tpy = r["n_trades"] / _y if _y > 0 else 0
                print(f"{r['name']:<12} | {r['n_trades']:>8} | {_tpy:>8.0f} | {r['alpha']:>10,.0f} | {r['max_dd']*100:>10.2f} | {r['flow_ann_util']*100:>10.2f} | {r['exhaust_count']:>4}")
            print("-" * 95)
    elif len(sys.argv) > 1 and sys.argv[1] == "weights":
        # 维持 baseline/combo2 配置，仅对比 LAYER_WEIGHTS：均权 vs 前重后轻 (1.5,1.2,1.0,1.0,0.8,0.8,0.7,0.5,0.5)
        WEIGHTS_NEW = (1.5, 1.2, 1.0, 1.0, 0.8, 0.8, 0.7, 0.5, 0.5)
        results = []
        for name, w in [("均权(baseline)", None), ("前重后轻", WEIGHTS_NEW)]:
            m = run_backtest(return_metrics=True, layer_weights_override=w)
            if m is None:
                continue
            m["name"] = name
            results.append(m)
        if len(results) < 2:
            print("weights: 回测未完成（请检查数据文件）")
        else:
            print("\n512890 层权对比（维持 combo2 配置，仅改 LAYER_WEIGHTS）")
            print("-" * 95)
            print(f"{'层权':<14} | {'成交笔数':>8} | {'Alpha(元)':>10} | {'最大回撤%':>10} | {'占用年化%':>10} | {'打满':>4}")
            print("-" * 95)
            for r in results:
                print(f"{r['name']:<14} | {r['n_trades']:>8} | {r['alpha']:>10,.0f} | {r['max_dd']*100:>10.2f} | {r['flow_ann_util']*100:>10.2f} | {r['exhaust_count']:>4}")
            print("-" * 95)
            print("均权: (1,1,1,1,1,1,1,1,1) ；前重后轻: (1.5,1.2,1.0,1.0,0.8,0.8,0.7,0.5,0.5)")
        ...
        print("提示: 得分 = 占用年化% + 0.3×回撤%；可手动将 MAX_LAYERS / LAYER_WEIGHTS 改为上表最优后运行 python mac_backtest_512890.py 查看详情。")
        ...
        print("提示: 运行 python mac_backtest_512890.py asymmetric 使用上述最优参数需在代码中设置 BUY_STEP_FACTOR / SELL_THRESHOLD_FACTOR。")
        ...
        print("提示: 将 GRID_STEP_FLOOR 设为最优值(如 0.0010 表示 0.10%) 后运行 python mac_backtest_512890.py")
        ...
        if results:
            ...
            print("-" * 115)
            print("说明: 使用当前数据文件（优先 5 年），单笔止盈 0.4%×涨多1.35，动态冷静期，ATR 熔断。年化与回撤均按该资金池规模计算。")
        ...
        # 不传 buy/sell_factor，与 cooling_off、tune 一致（1.0/1.0）；传 1.35 会拉高止盈、笔数从 210 降到 68
        run_backtest()
